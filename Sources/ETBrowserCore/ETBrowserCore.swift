...
public func findPreviews(skipping skipTypes: Set<String> = Set<String>()) -> [PreviewType] {
  return getPreviewTypes().filter { !skipTypes.contains($0.name) }.compactMap { conformance -> PreviewType? in
